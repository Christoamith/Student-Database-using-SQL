...
--CREATE CLUSTERED INDEX PK_AddressID ON LMT_University.enrol.Address (AddressID);
...
--DELETE FROM [enrol].[Address] WHERE AddressID=1;
...
SELECT StudentID, StudentFirstName, StudentLastName, StudentDOB, StudentMobile FROM [enrol].[Student] WHERE AddressID = 7 AND StudentRollNo = 7;
...
L.LecturerID, L.LecturerName, L.LecturerHighestQualification, DATEDIFF(YEAR,[LecturerAge],getdate()) AS LecturerYearService
		FROM [enrol].[Student] AS S LEFT JOIN [enrol].[Department] AS D ON S.DepartmentID = D.DepartmentID 
...
D.DepartmentID, D.DepartmentName, D.DepartmentDescription, D.DepartmentCapacity,
L.LecturerID, L.LecturerName, L.LecturerHighestQualification, DATEDIFF(YEAR,[LecturerAge],getdate()) AS LecturerYearService
		FROM [enrol].[Student] AS S LEFT JOIN [enrol].[Address] AS A ON S.AddressID = A.AddressID
		INNER JOIN [enrol].[Department] AS D ON S.DepartmentID = D.DepartmentID
		INNER JOIN [enrol].[Lecturer] AS L ON L.DepartmentID = D.DepartmentID;
...
--INSERT INTO [enrol].[AddrCopy] SELECT * FROM [enrol].[Address];
...
UPDATE [enrol].[Student] SET DepartmentID = 3 WHERE DepartmentID IN
(SELECT St.DepartmentID FROM [enrol].[Student] St
INNER JOIN [enrol].[Department] Dt ON St.DepartmentID=Dt.DepartmentID
WHERE St.StudentID = 42)
